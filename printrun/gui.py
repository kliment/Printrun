...
class LeftPane(wx.GridBagSizer):
    ...
    def __init__(self,root):
        ...
        htemp_choices=[root.temps[i] for i in sorted(root.temps.keys(),key=lambda x:root.temps[x])]
        ...
        root.htemp=wx.ComboBox(root.panel,-1,
                choices=htemp_choices,style=wx.CB_DROPDOWN,size=(70,-1),validator=NumericObjectValidator())
        ...
        btemp_choices=[root.bedtemps[i] for i in sorted(root.bedtemps.keys(),key=lambda x:root.temps[x])]
        ...
        root.btemp=wx.ComboBox(root.panel,-1,
                choices=btemp_choices,style=wx.CB_DROPDOWN,size=(70,-1),validator=NumericObjectValidator())
